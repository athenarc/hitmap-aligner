...
int main(int argc, char *argv[])
{
	...
	if (true)
	{
		...
		if (idx_file == "")
		{
			...
			if (listDir(data_folder, filenames, dirnames))
			{
				cout << "\t"
				...
				{
					if (cur_file->find(data_file) != std::string::npos && (cur_file->find(".widx") != std::string::npos || cur_file->find(".w2idx") != std::string::npos || cur_file->find(".hidx") != std::string::npos || cur_file->find(".h2idx") != std::string::npos))
					{
						ifstream fin3;
